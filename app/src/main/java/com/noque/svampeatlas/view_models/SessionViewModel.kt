...
class SessionViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun getUser(token: String) {
        viewModelScope.launch {
            val result = RoomService.getInstance(getApplication()).getUser()

            result.onError {

                DataService.getInstance(getApplication()).getUser(TAG, token) {
                    it.onSuccess {
                        viewModelScope.launch {
                            RoomService.getInstance(getApplication()).saveUser(it)
                        }
    ...
    private fun getNotifications(token: String) {
        ...
        DataService.getInstance(getApplication()).getUserNotificationCount(TAG, token) {
            it.onSuccess {
    ...
    private fun getObservations(user: User) {
        ...
        DataService.getInstance(getApplication()).getObservationCountForUser(TAG, user.id) {
            it.onSuccess {
                lastUpdated = Date(System.currentTimeMillis())
